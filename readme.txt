...
24.11.2021(84)
Исправил еще одну ошибку. Если в семье умирали оба родителя и оставались несовершеннолетние дети,
...
================================================================================
Задачи:
  1) удалить атрибуты father, mother. Довнедрить социальных родителей.
  2) Полумать о том, стоит ли удалять human.spouce. Ведт у человека есть семья, а в семье написано,
где жена, гже муж. Надо посмотреть, в каких областях spouce используется и попробовать вырезать.
  3)Разобраться с распределением еды. Для начала, чтобы 999% рациона не выдавало:
1038:1| KOMUABE|---- питание ---------
1038:1| KOMUABE|Запасы:    0.0
1038:1| KOMUABE|Первонаяальное распределение:
1038:1| KOMUABE|	 глав (  3)| 0000064| 100.0% -   0.0 еды
1038:1| KOMUABE| глав | 0000064| доля:   0.0| идеально:  36.0 взято из запасов:   0.0| остаток:    0.0|
KOMUABE| осталось запасов:0.0
1038:1| KOMUABE|окончательное распределение:
1038:1| KOMUABE|	 глав (  3)| 0000064| 999.0% -   0.0 еды


25.11.2021(85)
Рещил немного поукрупнять классы. Мне не нравится, что у класса Human очень большое количество
атрибутов. Надо их распихать по другим классам. Например birth_date и death_date я переместил
в класс Age, что выглядит довольно логично. Теперь Age содержит текущий возраст, возрастную стадию,
даты рождения и смерти.

25.11.2021(85)
От атрибута Human.spouse просто так не избавишься. Он не вполне дублируется переменными
Family.husband и Family.wife. husband и wife - это текущие супруги, но надо хранить списки
предыдущих супругов, а так же даты свадеб и разводов. Надо Human.spouse заменить на класс Spouses,
в котором хранить всех супругов и даты.
Вроде добился адекватной работы. Но нужно еще переписать отчет о супругах для hall_of_fame. Но уже
спать пора. Я пока его закомментировал. Завтра доделаю.

27.11.2021(86)
Доделал вывод сводки по человеку (Human.necrolog). Получилось более современно и логично.

Надо отделить биологических детей от социальных. Сейчас у меня так получается, что в отцы
записывается текущий муж роженицы. Но у меня есть список Family.dependents. Где происходит то же
самое. Дело в том, что при рождении ребенку надо присвоить отчество и фамилию. А это присвоение
происходит по атрибуту self.father. Если в father класть биологического отца, бусто будут
происходить ошибки. Короче, имя и отчество - эта штука социальная, поэтому надо переписать класс
присвоения имени под семью.

28.11.2021(87)
Переделал метод генерации имени человека, сами имена фамилии сделал в виде словарей, чтобы к ним
проще было иметь доступ через пол человека. Получилось более компактно. В этом методе немножко ушел
от Human.father. Надо и дальне от него избавляться. В следующих правках постараюсь избавиться от
mother и father. Должны быть только биологические и социальные родители.

01.12.2021(88)
Кстати, при зачатии ребенка можно передавать не мать, а семью, а там уже симметрично присваивать
родителей. Что я и сделал. В класс эмбриона перелается семья. На основе семьи создаются родители и
присваиваются эмбриону. Никаких отдельных отца и матери не создается. Родственные связи берутся из
класса Parents.

Короче, на пути вычищения атрибутов father и mother имеется препятствие. Это класс Parents.
Непонятно, что это за родители. Биологические или социальные.
По идее этот класс может содержать как социальных, так и биологических родителей. Это просто
структура хранения данных, которая возвращает папу и маму.
Факт зачатия тоже неплохо бы учитывать в статистике персонажа. А то сейчас в качестве возраста
эмбриона используется обычная дата.

11.12.2021(89)
Социальные родители - это родители в данный момент, мгновенные родители. Если мать в разводе, у
ребенка один соц родитель. У взрослого человека соц родителей нет. В таком случае надо сделать
список мам и пап за всю историю человека. Чтобы заботиться о последних в списке родителях. Но зачем
тогда соц родители?
На первых порах можно не отслеживать полный список меняющихся родителей, и не обнулять социальных
родителей при взрослении человека, а просто не трогать соц родителей в момент взросления. Это и
будут престарелые родители, о которых будет заботиться взрослый человек.
Нужно переписывать Family.parents, потому что в свете вышеописанного этот атрибут дублирует
Human.social_parents.
Тяжеловато было вычистить human.father human.mother, потому что трудно было уследить за мрущими
старыми родственниками. Если человек взрослеет, он сохраняет своих социальных родителей, но связь с
ними теряет. Он уже самостоятельная семья и из-за этого родители перестают отслеживаться. А их при
наличии лишней еды нужно подкармливать каждый ход. Но они к тому времени могут уже умереть, а их
повзрослевший ребенок об этом не знает.
Пришел  к тому, что нужно каждый ход обновлять social_parents для каждого человека. А это довольно
затратно по ресурсам. Нужно переделать.
На будущее нужно, чтобы у человека был список всех социальных детей, которые покинули его семью.
И когда этот человек умирает, этим детям посылается сигнал, что мертвого старика из социальных
родителей нужно убрать. А не поверять , живы ли родители каждый ход.


19.12.2021(90)
Такое ощущение, что гены наследуются неправильно. Или неправильно отображается, от кого они
наследуются. На зафиксированном сиде все гены наследуются в основном у отца. Как у мальчиков, так и
у девочек.
Проверил, это действительно так, все дети наследуют гены в основном от отца. Там был кусок кода,
когда надо было для наследования выдавать родителя одного с ребенком пола. Или наоборот
противоположного. Онв в любом случае выдавал отца. Переписал через класс Parents: добавил два
метода:  same_gender_parent и  opposite_gender_parent.
Так же сделал вывод отчета о социальных родителях в некролог. Подумалось,  что неплохо бы указывать
причину прекращения опекунства. Но для этого нужно модифицировать класс HumanRec, добавив в него еще
одно поле. Так и сделал, получился класс HumanRecCause, с новым полем cause, которому можно присвоить
значения из перечисления ParentCause.


05.01.2022(91)
Более-менее разобрался с социальными родителями. Как я говорил, это списки родителей
соответствующего пола. С датой начала и окончания опекунства. Тяжело социальных родителей получилось.
У меня для этого используются три метода:
    1) update - основной метод, добавляющий новых родителей при начальной инициализации человека и
при свадьбе. А так же завершающий родительство мужа при разводе.
    2) check_parents_alive  - метод, завершающий родительство при смерти родителя
    3) finish_parents - метод, завершающий родительство, когда ребенок достигает зрелости.
Все эти методы вызываются из разных мест, они ориентированы на разные обстоятельства. Что вносит
дополнительные сложности с пониманием, что происходит. Возможно, будет лучше сделать единый метод,
более громоздкий и тратящий много времени впустую, зато с более прозрачной логикой программы.

И все же пока не получилось реализовать стариков у взрослых детей. Чтобы взрослые дети подкармливали
престарелых родителей, не состоящих в вих семьях. Вернее, это не так трудно сделать. Нужно просто
кормить последних в списке родителей. Но при этом нужно постоянно отслеживать, живы ли они. Потому
что у меня нет такой отметки: последний социальный родитель умер, можно перестать отслеживать его.
А заводить еще ряд переменных, отслеживающих состояние бывших родителей, мне бы не хотелось.

06.01.2022(92)
Переделал отчет о супругах в том же стиле, в котором делал отчет о социальных родителях. Изначально
у меня был датакласс HumanRec, который вмещал человека, даты начала и конца. Под соц родителей я
сделал класс  HumanRecCause, в который добавил еще причину. То есть человек, дата начала опекунства,
дата прекращения опекунства и причина прекращения. Список причин содержался в перечислении
ParentCause. Сделано эо для того, чтобы не вычислять причину постфактум, а записывать ее сразу.
Но в отчете о супругах тоже указывается причина прекращения брака: развод или смерть одного из
супругов. Поэтому класс супругов я решил переделать в том же стиле.

09.01.2022(93)
Я думаю, к семейному распределению еды можно добавить третий этап. На втором этапе еда распределяется
только внутри семьи, старики не учитываются. И если после второго этапа у семьи останеся какая нибудь
лишняя еда, можно уже пожертвовать ее старикам. Тем более, уже будет понятно бедствуют старики или
нет. Может им никакая еда не нужна. Может быть это они захотят помочь детям. Так сказать, на третьем
этапе происходит выравнивание градиента сытости.
Прошелся по методу distribute в классе FamilySupplies, там, где семейные запасы еды распределяются
на членов семьи. Убедился что он работает правильно. Этот класс перегружен выводом данных в файл. Я
отделил вывод в файл от непосредственно расчетов. Но получилось не очень красиво. Та как там
несколько этапов расчета и для каждого этапа нужно было написать отдельный метод вывода информации.
Когда убедился, что считает правильно, написал версию этого метода, который ничего не выводит.
Получилось гораздо компактнее. Но все равно стоило бы разбить его еще на несколько методов.
...
Концепция общения. Допустим, есть четыре темы, на которые могут говорить люди. Вернее, четыре типа
сигналов, которые эти люди издают. Это наука, юмор, драма и героика (приключения). У каждого
человека есть изначальное отношение к этим темам, врожденное так сказать, которое формируется
случайно. Какую-то тему человек любит, к какой-то относится нейтрально, а какую-то ненавидит. На
ту тему, которая ему нравится, он говорит часто, а на те, что не нравятся - реже.
Предпочтения генерятся так: отношение к трем темам генерится из нормального распределения, а
четвертая вычисляется так, чтобы сумма четырех величин равнялась нулю. То есть не может быть
положительное отношение ко всем четырем темам. Или только отрицательное.
Может так случиться, что человек почти ко всему равнодушен. А может и так, что к трем темам
отношение резко отрицательное, зато в четвертой теме, например науке, он просто гений. И по ней
имеет великий дар убеждения. Зависит от дисперсии случайной величины.
Если отношение к теме у человека ровные, то и сигналы испускаемые им будут очень слабые. Если
человек одержим какой то темой, он будет спускать мощные сигналы (а что если тут присобачить радиус
распространение сигнала с постепенным затуханием?), будет легко увлекать людей и перекрещивать в
свою религию.
Впрочем, иногда он будет высказываться и по отрицательным темам. При этом и посыл будет
отрицательный. Если человек с отрицательным отношением к шуткам будет шутить, окружающие будут
разочаровываться в юморе. Опять же может получиться великий разоблачитель.
Окружающие воспринимают эти сигналы. Если он слышит разговор на тему, которая ему нравится, он
проникается симпатией к собеседнику. Если не нравится - антипатией.
Ребенок, слушающий эти сигналы может корректировать свое изначальное мировоззрение. Пока он
маленький, он перенисает любимые темы от родителей. Но чем он становится старше. Тем труднее
корректируется его мировоззрение.
Супруги, сходящиеся на общих интересах будут жить счаствиво и никогда не разойдутся.
Так же можно заводить друзей езе с детства. Человек слушает чужие сигналы и у него в голове
выстраивается рейтинг симпатии к людям. И он впоследствии будет чаще взаимодействовать с теми, к
то ему интересен. А если долго с человеком не общаться, рейтинг будет стремиться к нулю - мне
кажется, тут неплохо подойдет формула полураспада.
Тут можно подумать на тему споров - обмен сигралов от исповедников разных точек зрения - с
последующей коррекцией этих точек зрения. Можно вычислять победителя в споре, который получит
моральное удовлетворение, а проигравший - расстройство.
Ну и предрасположенность к определенным темам будут корректировать характеристики человека. Наука
отвечает за интеллект, эффективное решение жизненных задач, но способствует мизантропии, тяге к
уединению. Приключения влияют на смелость и решительность, увеличивают лидерские качества. Юмор
помогает расположить к себе и отвечает за жизнерадостный настрой. А драма - это обман, манипуляция,
тонкие, лиричные чувства и бесконечные жалобы, жалобы, жалобы...

Еще мыслишки по взаимодействию между персонажами...
Чтобы персонажи делали друг другу хорошее и плохое. Забирает еду - делает плохое дело, дает - делает
хорошее. И у человека изменяется мнение о том персонаже, с которым он взаимодействует. Ну и сделать
массив мнений о других людях у каждого человека. Ограничить его 40-50 персонами. При перегрузе
персоналии, к которым отношение близко к нулю, удаляются. Отношение может быть как положительным,
так и отрицательным. Если люди долго не взаимодействую, отношение со временем сходится к нулю. Типа,
они забывают друг друга. Причем можно для разных персонажей сделать скорость забывания плохих и
хороших поступков.
Помимо непосредственно причинения добра и зла можно сделать так, чтобы персонаж периодически
рассказывал окружающим о своем отношении к тому или иному человеку. И тем самым изменял мнение других.